import type { GetCurrentParams } from 'OpenWeatherMap';
...
function getLocaleLanguage(bcpTag: Intl.BCP47LanguageTag) {
  try {
    return new Intl.Locale(bcpTag).language;
  } catch (e) {}
...
function getLocaleRegion(bcpTag: string) {
  let region = regions[bcpTag];
  if (region) return region;

  try {
    return new Intl.Locale(bcpTag).maximize().region;
  } catch (e) {}
...
export function getLangParam(bcpTag: string): GetCurrentParams['lang'] {
  ...
  return language as GetCurrentParams['lang'];
}
