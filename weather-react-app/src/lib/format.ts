...
export const dateFormat = (date: string | number | Date, language: string) =>
...
export const relativeFormat = (d: string | number | Date, language: string) => {
  try {
    ...
    return now[getLanguageKey(language)].toLowerCase();
  } catch (e) {
...
export const formatTemperature = (
  ...
  try {
    ...
    const unitPart = formatter
      .formatToParts(c)
      .find((p) => (p.type as typeof p.type | 'unit') === 'unit');
    ...
    if (['ar', 'hi'].includes(languageKey))
      unitPart = units[languageKey][unit === 'fahrenheit' ? 'f' : 'c'];
    return [
...
const units: { [key: string]: { c: string; f: string } } = {
  ar: { c: '°م', f: '°ف' },
  hi: { c: '°से॰', f: '°फ़ेरन' },
  en: { c: '°C', f: '°F' },
};
const now: { [key: string]: string } = {
  en: 'now',
  eu: 'orain',
  am: 'አሁን',
  ceb: 'karon',
